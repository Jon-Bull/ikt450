...
input_dim = 784 #28*28
encoding_dim = 32#2#32#int(784/2)  # 32 floats -> compression of factor 24.5, assuming the input is 784 floats
...
#x_test = np.array([[float(random.randint(0,1)) for i in range(input_dim)]]*10000)
	...
	for i in range(n):
	    ...
	    plt.imshow(np.array(encoded_img[i].detach().numpy()).reshape(encoding_dim, 1))
	    plt.gray()
	    ax.get_xaxis().set_visible(False)
	    ax.get_yaxis().set_visible(False)
